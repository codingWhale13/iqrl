...
# === Specify W&B Run IDs===
mt_algos = ["MultiQRL", "TD-MPC2 w/o MPC", "W/o FSQ & w/o CE", "W/o CE"]
MT_RUN_IDS_ALL = {
    # NOTE: Only tricky-3 benchmark has the last two algos!
    BENCHMARKS_ALL[0]: {
        mt_algos[0]: ["c9jkk4o9", "fdtmlupm", "hxo9e892", "f6fky7wj", "eonoz2ng"],
        mt_algos[1]: ["pt4qtojc", "zsngc1mx", "rova5e9n", "mhpkeet9", "opp6nf56"],
        mt_algos[2]: ["f86wwbif", "7862249j", "9x2rt8p2", "oqtf9bgu", "1q9yt9yl"],
        mt_algos[3]: ["4p96cwdi", "5xeg7y5j", "ig86y66r", "6w5i99lx", "sa1d6pak"],
    },
    BENCHMARKS_ALL[1]: {
        mt_algos[0]: ["1zp9zqip", "ra7b5sr3", "aenmg9ig", "7150h1oc", "v5wkvaut"],
        mt_algos[1]: ["ueufardu", "d2h05vbt", "svwvvk2w", "z2x27zqf", "8s5vfiwa"],
    },
    BENCHMARKS_ALL[2]: {
        mt_algos[0]: ["me4qpu7w", "lqd17o0w", "nbwt66h6", "h3n98fmj", "shuxbe3r"],
        mt_algos[1]: ["ye2ldolb", "g2zxgbgy", "ud6uothg", "a0jn1j7q", "d2iaolgf"],
    },
    BENCHMARKS_ALL[3]: {
        mt_algos[0]: ["eqg4d9zz", "joigd2cd", "41vruqcy", "aod9te08", "apgkkvmz"],
        mt_algos[1]: ["atk38eck", "4crzssmy", "z0f0wnxn", "1bpsvau6", "z4rkd271"],
    },
    BENCHMARKS_ALL[4]: {
        mt_algos[0]: ["qduq3258", "0ohltkcw", "15t6dphm", "78vm2o3k", "yqk11i6w"],
        mt_algos[1]: ["fq7n36l3", "qx2f40xt", "9bfd9ajd", "dyutz0o9", "ng2ieem0"],
    },
}
# Change order to avoid confusion (Starting with "w/o")
mt_algos = ["MultiQRL", "W/o CE", "W/o FSQ & w/o CE", "TD-MPC2 w/o MPC"]
...
for benchmark in BENCHMARKS_ALL:
    ...
    if benchmark != "Tricky-3":
        # Create "invisible data" for algos without runs
        null_data = np.copy(normalized_scores["MultiQRL"])
        null_data[:] = np.nan
        normalized_scores["W/o FSQ & w/o CE"] = null_data
        normalized_scores["W/o CE"] = null_data
